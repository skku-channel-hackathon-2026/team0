...
DB 변경은 `cloudflare/migrations/`의 새 SQL을 PR에 포함하세요. main CI 성공 후 원격 적용이 먼저 실행되고 앱이 배포됩니다.
...
3. main CI가 성공하면 웹훅이 운영자 배포 시스템을 즉시 실행해 빌드·원격 DB 마이그레이션·배포합니다. GitHub 실행 대기와 빌드 시간은 필요합니다.
